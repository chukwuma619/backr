...
export const metadata: Metadata = {
  title: "Discover creators · Backr",
  description:
    "Explore and support creators on Backr. Browse by topic, trending creators, and new arrivals.",
};

const DISCOVER_PATH = "/discover";

export default async function MarketingDiscoverPage({
  searchParams,
}: {
  ...
  const requireWalletConnect = !user;
  ...
  const [
    ...
    user
      ? getRecommendedCreatorsForUser(user.id, 12)
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 space-y-10">
      <div>
        ...
        <div className="mb-8">
          ...
          <DiscoverTopics discoverPath={DISCOVER_PATH} />
        </div>
      </div>
      ...
      {isSearchOrTopic ? (
        <section>
          <h2 className="text-lg font-semibold tracking-tight mb-4">
            {topicLabel ? `Creators in ${topicLabel}` : "Search results"}
          </h2>
